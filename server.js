...
async function initDb() {
    try {
        ...
        if (adminExists.rows.length === 0) {
            ...
            await pool.query('INSERT INTO users (username, password, game_nick, role) VALUES ($1, $2, $3, $4)', ['admin', hash, 'Admin', 'admin']);
...
app.post('/api/register', async (req, res) => {
    try {
        ...
        await pool.query('INSERT INTO users (username, password, game_nick) VALUES ($1, $2, $3)', [username, hash, gameNick || username]);
...
app.post('/api/login', async (req, res) => {
    try {
        ...
        res.json({ success: true, user: { id: user.id, username: user.username, gameNick: user.game_nick, role: user.role } });
